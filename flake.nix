{
  description = "FLibrary Flavored TeX template";
  inputs = {
    nixpkgs.url = "github:NixOS/nixpkgs/nixos-unstable";
    utils.url = "github:numtide/flake-utils";
    texutils.url = "github:Ninlives/texutils.nix";
  };
  outputs = { self, nixpkgs, utils, texutils }:
    with utils.lib;
    with nixpkgs.lib;
    with builtins;
    {
      overlays.default = final: prev: {
        texlive = prev.texlive // {
          notestex.pkgs = [ (prev.callPackage ./pkgs/notestex { }) ];
        };
      };
    } // (
          ...
          apps = rec {
            fmt = utils.lib.mkApp {
              drv = with import nixpkgs { inherit system; };
                pkgs.writeShellScriptBin "euler-fmt" ''
                  export PATH=${
